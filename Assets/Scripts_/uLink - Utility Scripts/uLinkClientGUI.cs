...
public class uLinkClientGUI : uLink.MonoBehaviour
{
	...
	public string gameType = "Shoot-Field";
	...
	void QuickGUI()
	{
		if (inputName)
		{
			GUILayout.BeginHorizontal();
			GUILayout.FlexibleSpace();
			GUILayout.Label("Let's Game Begin:");
			GUILayout.FlexibleSpace();
			GUILayout.EndHorizontal();
            ...
            if (GameManager.Instance.cPlayer != null)
                playerName = GameManager.Instance.cPlayer.name;
            else
                playerName = "Guest" + Random.Range(100, 100000);
