...
public class Joystick : MonoBehaviour
{
    ...
    void  Start (){
	    ...
	    if (touchPad) {
            ...
            Vector2 minVec = guiBoundary.min;
            minVec.x = defaultRect.x - guiTouchOffset.x;
            minVec.y = defaultRect.y - guiTouchOffset.y;

            Vector2 maxVec = guiBoundary.max;
            maxVec.x = defaultRect.x + guiTouchOffset.x;
            maxVec.y = defaultRect.y + guiTouchOffset.y;

            guiBoundary.min = minVec;
            guiBoundary.max = maxVec;
	    }
    }
    ...
    void  Update (){
	    ...
	    if (!touchPad) {
            ...
            rect.x = (position.x - 1) * guiTouchOffset.x + guiCenter.x;
            rect.y = (position.y - 1) * guiTouchOffset.y + guiCenter.y;
            gui.pixelInset = rect;
	    }
    }
